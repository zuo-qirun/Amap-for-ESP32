...
这是一个独立项目，用轻量 Android 转发 App 监听高德地图车机版广播，聚合成完整导航状态快照，再通过 Wi-Fi UDP 或 BLE 发给 ESP32-S3。ESP32-S3 负责接收 JSON、维护硬件无关的导航状态，并在 OLED/TFT 上显示。
...
5. `Esp32UdpForwarder` 节流发送，并按设置选择 UDP 或 BLE；BLE 会自动扫描 `AMap-ESP32-*`，无需系统配对。
