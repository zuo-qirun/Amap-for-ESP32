...
触控版在自动、导航、音乐、设备状态四个页面使用同一套手势：左右滑循环切换页面，上滑直达设备状态，下滑或长按返回自动模式，轻点显示当前页面提示。横向拖动有 10 像素方向锁定阈值，页面与手指 1:1 移动；松手后结合距离和速度完成切换或弹回，动画可以再次触摸打断。

`Config.h` 中的上述宏是首次启动或 NVS 尚无记录时的默认值。烧录后也可在设备配置网页的“显示硬件”区域切换 ST7789V / ILI9341V，并单独开启或关闭 FT6336U 触摸和屏幕反色；保存后设备会自动重启并从 NVS 加载新设置。无触摸版建议保持关闭，避免初始化和占用触摸相关 GPIO。
...
TFT 针对 N16R8 使用三帧 PSRAM + 内部 DMA 分块缓冲：当前帧、上一帧和手势相邻页位于 8 MB PSRAM，16 KB 传输缓冲位于内部 DMA RAM。渲染器以 16×8 像素脏块比较前后帧，把相邻变化合并成矩形后批量发送；变化超过约 45% 或过于碎片化时自动切换整帧发送。SPI 使用 ESP32-S3 可实际产生的 80 MHz 时钟，逻辑刷新周期为 33 ms。
