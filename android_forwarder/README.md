...
轻量 Android 转发 App，负责监听高德地图车机版广播、聚合导航状态，同时自动读取标准音乐播放器的播放状态与歌词，并通过 UDP 或 BLE 发送给 ESP32-S3。
...
- `MusicNotificationListener.java`：通过通知使用权读取标准 `MediaSession`，优先选择正在播放的会话，并识别主流音乐应用。
...
安装后授予通知权限；使用 BLE 时还需授予“附近设备”权限。若要读取音乐播放器，还需在 App 内点击“打开音乐读取权限”，并在系统“通知使用权”页面允许本 App。该特殊权限与普通通知权限不同。服务是前台服务，不需要悬浮窗权限。

播放进度来自 Android 标准 `MediaSession`。当前内置网易云、QQ 音乐、酷狗、酷我、Spotify、汽水音乐、咪咕音乐、小米音乐、华为音乐、Apple Music、YouTube Music 和 Amazon Music 的来源识别，其他规范播放器会以通用来源自动接入。歌词统一使用网易云搜索与歌词接口：网易云歌曲可直接使用歌曲 ID，其他来源按歌名、歌手和时长匹配；优先解析 YRC 逐字时间轴，没有 YRC 时回退普通 LRC，下载后缓存 30 天。接口失败时仍会发送歌名、歌手、播放状态和进度。音乐活动时使用 200 ms 心跳，并通过较小的 `music_update` 包降低 BLE 分片延迟；ESP32 会利用当前词和整句时间戳在两次心跳之间本地插值。

配置页使用连续表单，按“连接设置 / 导航设置 / 音乐播放器 / ESP32 设备设置 / 开发者选项”依次组织。歌词延迟校正范围为 `-5000` 到 `+5000` ms：正数让歌词提前，负数让歌词延后，建议每次调整 100–200 ms。
...
5. 打开任意支持标准 `MediaSession` 的音乐应用播放歌曲；App 状态页应显示播放器名称、歌名与歌词状态。UDP 和 BLE 共用同一 JSON 编码，都会携带 `music` 对象。
...
- `音乐读取: 未授权`：在系统“通知使用权”中启用“音乐播放状态读取”。
