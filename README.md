...
当前 8 MB OTA 分区中，固件约为 1.22 MB。常见 4 MB 双 OTA 分区的单个 app 槽仅为 `0x140000`（1.31 MB），当前固件虽然能装下，但余量约 90 KB；不适合再内置完整位图资源包，也会限制后续 TFT 驱动和字体升级。网页预览与 TMC 色条目前全部由 CSS/几何图形绘制，不占资源文件空间；TFT 也应优先用几何绘制、少量图标和按需字体。若要保留双 OTA 且加入大量中文字体/PNG 图标，建议使用 8 MB Flash；4 MB 方案则应取消大资源，或关闭双 OTA 并重新设计分区。
