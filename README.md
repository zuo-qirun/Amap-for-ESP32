...
TFT 界面使用了 Navi-Link 的资源与布局作为参考。来源、使用范围及上游许可证状态详见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。第三方材料不因收录于本仓库而自动适用本项目许可证。
...
项目也支持鸿讯 28005 2.8 英寸 240x320 SPI TFT：显示控制器可在 ST7789V 与 ILI9341V 间选择，电容触摸版本的 FT6336U 通过 I2C 接入。完整引脚表和配置方法见 [ESP32 显示端文档](esp32_display/README.md#鸿讯-28005-spi-tft-与电容触摸)。

屏幕芯片、触摸开关和屏幕反色也可以在 ESP32 配置网页中修改，保存后自动重启生效；当前默认配置为 ST7789V、无触摸。

启用 FT6336U 后，TFT 各界面支持统一手势：左右滑在自动、导航、音乐、设备状态四个页面间循环切换，上滑直达设备状态，下滑或长按返回自动模式，轻点显示当前页面提示。横向切换会跟随手指，并依据滑动距离和释放速度决定完成切换或回弹。显示设置中的“音乐风格”可在标准、PiPWindow 和 Refined 三种播放页之间循环选择；Refined 页面采用封面驱动的沉浸背景、错峰歌词行切换和居中的播放控制。
...
4. `MusicNotificationListener` 通过通知使用权自动读取标准 `MediaSession`，优先选择正在播放的会话，并识别网易云、QQ 音乐、酷狗、酷我、Spotify、汽水音乐等主流应用。歌词客户端使用网易云曲库按歌曲 ID（仅网易云）或歌名/歌手匹配并缓存 LRC/YRC 时间轴歌词；支持 25 FPS 平滑逐字高亮、长歌词提前滚动和可配置的正负延迟校正。
5. `Esp32Protocol` 把导航和音乐快照序列化为协议 v1 JSON。
6. `Esp32UdpForwarder` 节流发送，并按设置选择 UDP 或 BLE；BLE 会自动扫描 `AMap-ESP32-*`，无需系统配对。

### ESP32 数据流

1. `NetworkManager` 监听 UDP `4210`；`BleReceiver` 同时广播 GATT 服务并重组 Android 发来的 BLE 分片。
...
- BLE 使用有响应 GATT 写入；单个 JSON 会按协商 MTU 分片，每片携带帧号、偏移、总长度和首尾标记，ESP32 完整重组后才解析。
- BLE 不依赖 Wi-Fi，也不需要在系统蓝牙页面配对；Android 12 及以上需授予“附近设备”权限。ESP32-S3 可与 Wi-Fi/AP 配网页分时共存；在同时传输大文件时 BLE 延迟可能升高。
- UDP 与 BLE 共用同一份协议 v1 JSON 和渲染链路，可在 App 设置页切换。
...
- `AMAP_TFT_DRIVER`：按 28005 实物选择 `AMAP_TFT_DRIVER_ST7789` 或 `AMAP_TFT_DRIVER_ILI9341`。
- `AMAP_TFT_*_PIN`：28005 显示 SPI 与背光引脚，默认接线见 ESP32 显示端文档。
- `AMAP_TFT_TOUCH_DRIVER`：电容版选择 `AMAP_TFT_TOUCH_DRIVER_FT6336U`，无触摸版选择 `AMAP_TFT_TOUCH_DRIVER_NONE`。
...
- 页面会显示当前 Wi-Fi 状态、STA IP、UDP 端口、BLE 状态/配对记录、配网热点状态，并可保存新的 Wi-Fi 名称和密码。
...
- 配置页可清除 ESP32 端保存的 BLE bond（当前 Android GATT 通常无需配对，因此通常显示为 0）；也可手动上传可信的 `firmware.bin`，上传成功后设备会自动重启。
...
- 通信方式：选择 `UDP` 或 `BLE`。BLE 会自动连接附近的 `AMap-ESP32-*` 开发板。
- ESP32 IP：填写 ESP32 串口打印出的本机 IP。
- UDP 端口：默认 `4210`，需要与 ESP32 一致。
- 目标高德应用：从已安装候选中选择；共存版未列出时可手动填写包名。
- 音乐读取权限：点击“打开音乐读取权限”，在系统“通知使用权”中允许本 App。
- 打开“启用转发服务”。
