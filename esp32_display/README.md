...
`AMAP_WIFI_SSID` / `AMAP_WIFI_PASSWORD` 是兜底配置。设备也支持 Web 配网，保存后的 Wi-Fi 会写入 ESP32 NVS，优先级高于 `Config.h`。
...
5. 连接成功后配网热点会关闭；之后可通过串口打印的 STA IP 访问同一个状态/配网页面。
...
GitHub Actions 不主动上传到服务器。国内服务器可定时从 GitHub Release 或 artifact 拉取产物：

```bash
# 从滚动 Release 同步 stable
python3 scripts/sync_ota_from_github.py \
...
3. 打开配置页面，点击“检查更新”可立即刷新 manifest。
4. 点击“立即升级”后，ESP32 下载 `firmware.bin`。
5. 固件下载完成后校验 SHA256。
6. 校验通过后通过 Arduino `Update` 写入 OTA 分区。
7. 写入成功后自动重启。

升级失败时，配置页面会显示 HTTP 错误、manifest 错误、大小不匹配、SHA256 校验失败或 `Update` 写入错误。

### dev 回退 stable

本工程已经使用 8MB Flash 双 OTA 分区表 `partitions_ota_8mb.csv`：
